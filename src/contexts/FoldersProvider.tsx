...
interface FoldersContextType {
  ...
  checkIfCoursePurchased: (folderId: number) => Promise<boolean>
  refreshFolders: () => Promise<void>
  getCategoryTitleById: (categoryId: number) => string
}
...
export const FoldersProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // פונקציה לשליפת שם הקטגוריה לפי categoryId
  const getCategoryTitleById = (categoryId: number): string => {
    ...
    return categoryFolder?.title || "קטגוריה לא ידועה"
  }
