...
//         (folder: Course) => folder.courseId === Number.parseInt(courseId || "0")
//       )

//       if (!foundCourse) {
//         console.warn(`לא נמצא קורס עם courseId=${courseId}`)
//         setCourse(null)
//         return
//       }
...
//         { length: foundCourse.numberOfLessons },
//         (_, i) => ({
//           lessonId: i + 1,
//           title: `שיעור ${i + 1}`,
//           description: `תיאור שיעור ${i + 1} בקורס ${foundCourse.title}`,
...
const CourseView: React.FC = () => {
  ...
  const loadCourseAndLessons = async () => {
    try {
      ...
      // שליפת כתובת המייל של המורה לפי teacherId
  ...
  // const sendEmailToTeacher = () => {
  //   if (!course || !course.teacherEmail) return
  //   const subject = encodeURIComponent(`שאלה לגבי הקורס: ${course.title}`)
  //   const body = encodeURIComponent(
  //     `שלום ${course.teacherName},\n\nיש לי שאלה לגבי הקורס "${course.title}".\n\nתודה,`
  //   )
  //   window.open(`mailto:${course.teacherEmail}?subject=${subject}&body=${body}`)
  // }
  const sendEmailToTeacher = () => {
