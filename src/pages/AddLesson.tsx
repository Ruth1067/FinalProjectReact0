...
const AddLesson: React.FC = () => {
  ...
  // עדכון ספירת שיעורים בקורס הנבחר
  useEffect(() => {
    if (!formData.courseId) {
      setLessonsCount(0);
      return;
    ...
    const selectedCourseId = Number(formData.courseId);
    // סינון רק שיעורים (lessonId !== null) ששייכים לקורס הנבחר
    const lessons = allFolders.filter(
      (folder) =>
        folder.courseId === selectedCourseId &&
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const courseIdNum = Number.parseInt(formData.courseId);
    const lessonIdNum = lessonsCount + 1; // מזהה שיעור חדש (מספר עוקב)

    const lessonData = {
      categoryId: null, // חשוב: לא לשייך קטגוריה כאן, או לשמור כ-null
      courseId: courseIdNum, // שייך לקורס קיים
      teacherId: user?.userId,
      lessonId: lessonIdNum, // מזהה שיעור, לא null
      title: formData.title,
      teacherName: user?.userName,
      description: formData.description,
    };
    ...
    try {
      setUploading(true);
      // מעלה קובץ עם courseId ו-lessonId לבניית שם קובץ אחיד בשרת/AWS
      await uploadApi.uploadFile(selectedFile, lessonData.courseId, lessonData.lessonId);
      setUploading(false);

      // שומר את השיעור עם lessonId מלא, כדי שיופיע רק כשיעור בתוך הקורס
      await folderApi.addLesson(lessonData);
      setSuccess(true);
        ...
        if (err.response) {
          console.log(
            err.response.data?.message ||
              `שגיאה: ${err.response.status} ${err.response.statusText}`
          );
        } else if (err.request) {
          console.log("לא התקבלה תגובה מהשרת. אנא בדוק את החיבור שלך.");
        } else {
