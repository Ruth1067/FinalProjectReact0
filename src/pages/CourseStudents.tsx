...
const CourseStudents: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadStudents = async () => {
      ...
      try {
        ...
        // טען פרטי קורס (כולל שם)
        const course = await folderApi.getFolderById(Number(folderId))
        setCourseTitle(course.title || "קורס לא ידוע")
      } catch (e) {
        setError("אירעה שגיאה בטעינת רשימת התלמידים או פרטי הקורס")
        console.error(e)
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <h1 className="text-2xl font-bold mb-4 flex items-center space-x-2 space-x-reverse">
        <Users className="h-6 w-6" />
        <span>תלמידים בקורס {courseTitle}</span>
      </h1>
